...
awful.screen.connect_for_each_screen(function(s)
    ...
    for i, t in ipairs(tags.setup) do
        if tags.mods[s_idx] and tags.mods[s_idx][i] then
            t = gears.table.join(t, tags.mods[s_idx][i])
        end
        -- Select the first tag.
        t.properties.screen = s
        if i == 1 then
            t.properties.selected = true
        end
        table.insert(tags[s_idx], awful.tag.add(t.name, t.properties))
    end
...
globalkeys = gears.table.join(
    ...
    awful.key({ "Mod1",           }, "Tab", histoggle,
              {description = "focus the previous client in history", group = "client"}),
...
awful.rules.rules = gears.table.join(awful.rules.rules, add_rules)
